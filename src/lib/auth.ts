...
// not an `organization_member` of the project's owning org. Drew's CRM
// customers stay separate from his TruArc team workspace.
//
...
async function createUserApiKey(sessionToken: string): Promise<string> {
  const res = await authedFetch(
    '/api/v1/api-keys',
    {
      ...
      // Project-scoped key: caller becomes a project_member of this app
      // (not an organization_member of the owning TruArc org). The project
